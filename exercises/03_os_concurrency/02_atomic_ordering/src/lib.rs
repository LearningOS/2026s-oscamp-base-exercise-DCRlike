...
//!
//! ## Key Concepts
//! - `Ordering::Relaxed`: No synchronization guarantees 什么都不做
//! - 读保证`Ordering::Acquire`: Read operation, prevents subsequent reads/writes from being reordered before this operation
//! - 写保证`Ordering::Release`: Write operation, prevents preceding reads/writes from being reordered after this operation
//! - ACQ+REL`Ordering::AcqRel`: Both Acquire and Release semantics
//! - 严格保证`Ordering::SeqCst`: Sequentially consistent (global ordering)
//!
