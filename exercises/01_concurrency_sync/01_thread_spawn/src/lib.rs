...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let sum_a = thread::spawn(move || a.iter().sum());
    let sum_b = thread::spawn(move || b.iter().sum());
...
pub fn named_sleeper(value: i32, ms: u64) -> i32 {
    ...
    let val = thread::Builder::new().name("sleeper".into())
    .spawn(move || {
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    ...
    let tuple = thread::scope(|s| {
        let sum_a = s.spawn(|| a.iter().sum::<i32>()).join().unwrap();
        let sum_b = s.spawn(|| b.iter().sum::<i32>()).join().unwrap();
        (sum_a, sum_b)
    });
    tuple
}
